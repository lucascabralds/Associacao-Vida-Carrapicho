// eventos-loader.js - Carrega e renderiza eventos do backend

const API_URL = 'http://localhost:3000/api';
const CORES_EVENTOS = ['event-card-green', 'event-card-gold', 'event-card-pink'];

// Função para formatar data
function formatarDataEvento(dataISO) {
    const data = new Date(dataISO);
    const dia = data.getDate().toString().padStart(2, '0');
    const mes = data.toLocaleString('pt-BR', { month: 'short' }).toUpperCase();
    return { dia, mes };
}

// Função para criar o HTML de um card de evento
function criarCardEvento(evento, indice) {
    const { dia, mes } = formatarDataEvento(evento.event_date);
    const corEvento = CORES_EVENTOS[indice % CORES_EVENTOS.length];
    const titulo = evento.title || 'Evento sem título';
    const descricao = evento.description || 'Confira os detalhes deste evento.';
    const imagem = evento.cover_image || './img/evento.png';
    
    // Se há múltiplas imagens, mostrar miniaturas
    const imagensHTML = evento.images && evento.images.length > 0
        ? evento.images.slice(0, 3).map(img => `
            <div class="event-thumb">
                <img src="${img.image_url}" alt="${titulo}" />
                <p>${titulo}</p>
            </div>
        `).join('')
        : `
            <div class="event-thumb">
                <img src="${imagem}" alt="${titulo}" />
                <p>${titulo}</p>
            </div>
        `;

    return `
        <article class="event-card" data-event-id="${evento.id}">
            <header class="event-card-header ${corEvento}">
                <div class="event-date">
                    <span class="day">${dia}</span>
                    <span class="month">${mes}</span>
                </div>
                <div class="event-name">${titulo}</div>
            </header>
            <div class="event-card-thumb-row">
                ${imagensHTML}
            </div>
            <div class="event-card-body">
                <h2>${titulo}</h2>
                <p>${descricao.substring(0, 120)}${descricao.length > 120 ? '...' : ''}</p>
            </div>
        </article>
    `;
}

// Função para carregar eventos do backend
async function carregarEventos() {
    try {
        const response = await fetch(`${API_URL}/events`);
        
        if (!response.ok) {
            throw new Error(`Erro ao buscar eventos: ${response.status}`);
        }
        
        const eventos = await response.json();
        const secaoProxEventos = document.getElementById('prox_event');
        
        // Se não há eventos, oculta a seção inteira
        if (!eventos || eventos.length === 0) {
            if (secaoProxEventos) {
                secaoProxEventos.style.display = 'none';
            }
            return eventos;
        }
        
        // Se há eventos, exibe a seção
        if (secaoProxEventos) {
            secaoProxEventos.style.display = 'block';
        }
        
        // Ordena eventos por data
        eventos.sort((a, b) => new Date(a.event_date) - new Date(b.event_date));
        
        // Limita a 3 eventos na página inicial
        const eventosExibicao = eventos.slice(0, 3);
        
        // Renderiza os eventos
        const containerEventos = document.getElementById('eventos');
        if (containerEventos && eventosExibicao.length > 0) {
            containerEventos.innerHTML = eventosExibicao
                .map((evento, indice) => criarCardEvento(evento, indice))
                .join('');
            
            // Atualiza o link "Ver Todos os Eventos"
            const btnVerTodos = document.querySelector('.event-cta-btn');
            if (btnVerTodos && eventos.length > 3) {
                btnVerTodos.href = './PROJETO CARRAPICHO/Projeto-carrapicho/frontend/evento.html?listar=true';
            }
        }
        
        return eventos;
    } catch (error) {
        console.error('Erro ao carregar eventos:', error);
        // Em caso de erro, oculta a seção
        const secaoProxEventos = document.getElementById('prox_event');
        if (secaoProxEventos) {
            secaoProxEventos.style.display = 'none';
        }
    }
}

// Carrega eventos quando o DOM estiver pronto
document.addEventListener('DOMContentLoaded', function() {
    // Aguarda um pouco para garantir que o backend está pronto
    setTimeout(() => {
        carregarEventos();
    }, 500);
});

// Opcionalmente, atualizar eventos a cada 30 segundos
setInterval(carregarEventos, 30000);
