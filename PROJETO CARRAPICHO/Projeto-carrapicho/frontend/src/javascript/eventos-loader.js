...
function criarCardEvento(evento, indice) {
    ...
    const { dia, mes } =
        formatarDataEvento(evento.event_date);

    const corEvento =
        CORES_EVENTOS[indice % CORES_EVENTOS.length];

    const titulo =
        evento.title || 'Evento sem título';
    ...
    const imagem =
        evento.cover_image || './img/evento.png';
    ...
    return `
        ...
        >
            ...
            <header
                class="event-card-header ${corEvento}"
            >
...
async function carregarEventos() {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(
                `Erro ao buscar eventos: ${response.status}`
            );
        }
        ...
        const secaoProxEventos =
            document.getElementById('prox_event');
        ...
        if (
            containerEventos &&
            eventosExibicao.length > 0
        ) {

            containerEventos.innerHTML =
                eventosExibicao
                    .map((evento, indice) =>
                        criarCardEvento(evento, indice)
                    )
                    .join('');
        }
