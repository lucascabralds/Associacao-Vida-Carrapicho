$(document).ready(function () {
  ...
  // =====================================================
  // TRANSPARÊNCIA - Busca do backend
  // =====================================================
  async function carregarDadosTransparencia() {
    try {
      const API_URL = window.location.origin + '/api';
      console.log('📡 Buscando transparência em:', `${API_URL}/transparency/stats`);

      const response = await fetch(`${API_URL}/transparency/stats`);
      ...
      console.log('✅ Gráfico de transparência atualizado!');
    } catch (error) {
  ...
  $(document).ready(function () {
    // Quando o modal de transparência abrir, recarrega os dados
    $('.abrir-modal-transparencia').on('click', function (e) {
      e.preventDefault();
      openModal();
      setTimeout(carregarDadosTransparencia, 300);
    });
  });
